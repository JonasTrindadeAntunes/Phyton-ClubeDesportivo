...
def insert_atleta(atleta):
    with conn:
        cursor_atleta.execute(
            "INSERT INTO Atletas VALUES (:nome, :numidenticivil, :genero, :idade, :atividade, :fcr, :premios)",
            {
                "nome": atleta.nome,
...
def update_premios(atleta, premios):
    with conn:
        cursor_atleta.execute(
            """UPDATE Atletas SET premios= :premios
...
def remove_atleta(atleta):
    with conn:
        cursor_atleta.execute(
            "DELETE FROM Atletas WHERE nome= :nome AND numidenticivil= :numidenticivil",
